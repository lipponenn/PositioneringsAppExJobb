...
class CoordinateCalculator(
    ...
    override fun onBeaconDistanceFound(beacon: Beacon, rssi: Int, txPower: Int) {
        if (beaconsCorners.containsKey(beacon)) {
            ...
            if (logs) {
                ...
                Log.i("CoordinateCalculator", "Found beacon $beacon, rssi: $rssi, txPower, : $txPower $allRssis")
            }
    ...
    fun recalcCoordinates(distancesMedium: MutableMap<Beacon, Double>) {
        ...
        if (logs) {
            ...
            Log.i("Calculated coord", String.format("{%.2f, %.2f}. Deviation=%s; Covariance=%s", x, y, standardDeviation, covarianceMatrix))
        }
    }
